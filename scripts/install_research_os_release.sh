#!/usr/bin/env bash
# Native Ubuntu source-release installer for Research OS.
# Fail-fast, non-interactive, release-pinned. Does not curl remote code.
# Does not print secrets. Does not open PostgreSQL or research-osd publicly.

set -euo pipefail

OPT_ROOT="/opt/research-os"
RELEASES_DIR="${OPT_ROOT}/releases"
CURRENT_LINK="${OPT_ROOT}/current"
ETC_DIR="/etc/research-os"
ENV_FILE="${ETC_DIR}/research-os.env"
STATE_DIR="/var/lib/research-os"
LOG_DIR="/var/log/research-os"
SERVICE_USER="research-os"
SERVICE_GROUP="research-os"
MIN_PY_MAJOR=3
MIN_PY_MINOR=11
...
usage() {
  cat <<'EOF'
Usage: install_research_os_release.sh --source DIR --release-id ID --python PYTHON [options]

Required:
  --source DIR       Immutable source tree to copy (local path only)
  --release-id ID    Release directory name under /opt/research-os/releases/
...
Optional:
  ...
  --enable           systemctl enable --now research-osd (implies --install-units)
...
if [[ ! -f "$SOURCE/pyproject.toml" || ! -f "$SOURCE/alembic.ini" ]]; then
  echo "source is not a Research OS tree (missing pyproject.toml or alembic.ini)" >&2
  exit 2
fi
...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  echo "copy config/research-osd.env.example and set the real DSN locally" >&2
  exit 2
fi
...
VERIFY_PY="${SOURCE}/scripts/verify_research_os_release.py"
if [[ ! -f "$VERIFY_PY" ]]; then
...
"${RELEASE_DIR}/.venv/bin/python" "${RELEASE_DIR}/scripts/verify_research_os_release.py" "${VERIFY_ARGS[@]}"
...
if [[ "$INSTALL_UNITS" -eq 1 ]]; then
  install -m 0644 -o root -g root \
    "${CURRENT_LINK}/deploy/systemd/research-osd.service" \
    /etc/systemd/system/research-osd.service
  install -m 0644 -o root -g root \
    "${CURRENT_LINK}/deploy/systemd/research-os-dashboard.service" \
    /etc/systemd/system/research-os-dashboard.service
  if [[ -d /etc/logrotate.d ]]; then
    install -m 0644 -o root -g root \
      "${CURRENT_LINK}/deploy/logrotate/research-os" \
      /etc/logrotate.d/research-os
  fi
  systemctl daemon-reload
fi

if [[ "$ENABLE" -eq 1 ]]; then
  systemctl enable research-osd.service
  systemctl start research-osd.service
fi
