"""Verify a Zest source release and host EnvironmentFile.
...
MIN_PYTHON = (3, 11)
EXPECTED_SCRIPTS = ("zestd", "zest-dashboard", "zest")
...
def assert_assets(root: Path) -> None:
    ...
    if 'zestd = "zest.interface.zestd:main"' not in pyproject:
        raise SystemExit("pyproject.toml is missing the zestd console script")
    if 'zest-dashboard = "zest.interface.dashboard:main"' not in pyproject:
    ...
    unit = (root / "deploy/systemd/zestd.service").read_text(encoding="utf-8")
    active = "\n".join(
    ...
    for item in required:
        if item not in active:
            raise SystemExit(f"zestd.service missing {item}")
    forbidden = (
        "0.0.0.0",
    ...
    for item in forbidden:
        if item in active:
            raise SystemExit(f"zestd.service must not contain {item}")
    dashboard = (root / "deploy/systemd/zest-dashboard.service").read_text(encoding="utf-8")
    if "--host 127.0.0.1" not in dashboard:
        raise SystemExit("dashboard unit must bind 127.0.0.1")
    if "User=zest" not in dashboard:
        raise SystemExit("dashboard unit must run as zest")
    if "0.0.0.0" in dashboard:
        raise SystemExit("dashboard unit must not bind publicly")
    example = (root / "config/zestd.env.example").read_text(encoding="utf-8")
    ...
    if "zest.qualification.staging_spine" not in fixture:
        raise SystemExit("vds fixture must import shipped qualification primitives")
    if "zest.qualification.j11_fencing" not in fixture:
...
def check_import(release_root: Path) -> None:
    ...
    completed = subprocess.run(
        [
            str(python),
            "-c",
            "import zest, zest.interface.zestd, "
            "zest.qualification.staging_spine, "
            "zest.qualification.j11_fencing; "
            "from zest.qualification.j11_fencing import cleanup_j11_owner",
        ],
        check=False,
        capture_output=True,
        text=True,
...
def check_entrypoint(release_root: Path) -> None:
    ...
    help_cmd = subprocess.run(
        [str(release_root / ".venv" / "bin" / "zestd"), "--help"],
        check=False,
        capture_output=True,
        text=True,
    )
    if help_cmd.returncode != 0:
        raise SystemExit("zestd --help failed")
    print("entrypoint=zestd")
...
def resolve_alembic_ini(values: dict[str, str], *, release_root: Path) -> Path:
    ...
    if candidate.is_file():
        return candidate
    candidate = (release_root / "src/zest/application/osd_settings.py").resolve()
    if candidate.is_file():
        from zest.application.osd_settings import resolve_alembic_ini as _resolve
...
def check_db(values: dict[str, str]) -> None:
    from zest.application.osd_settings import load_osd_settings
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="verify_zest_release")
