"""zestd SIGTERM / SIGINT process-exit contract against real PostgreSQL."""
...
LISTEN_PREFIX = "zestd listening on "
...
def _wait_for_listen(proc: subprocess.Popen[str], timeout: float = 15.0) -> str:
    ...
    while time.time() < deadline:
        if proc.poll() is not None:
            ...
            raise AssertionError(f"zestd exited early rc={proc.returncode} stderr={stderr}")
        ...
        if LISTEN_PREFIX in line:
            ...
            if url.endswith(":0"):
                raise AssertionError(f"zestd published unbound port: {url}")
            return url
    raise AssertionError(f"zestd did not print listen address: {buf}")
...
def _stop_and_collect(
    ...
    *,
    ...
    try:
        ...
        raise AssertionError("zestd did not exit after signal within bound")
...
class ZestdSigtermTests(unittest.TestCase):
    engine = None
    ...
    def _spawn(self) -> subprocess.Popen[str]:
        assert TEST_URL is not None
        env = dict(os.environ)
        env["ZEST_DATABASE_URL"] = TEST_URL
        env["ZEST_BIND_HOST"] = "127.0.0.1"
        env["PYTHONUNBUFFERED"] = "1"
        env["PYTHONPATH"] = str(_SRC) + os.pathsep + env.get("PYTHONPATH", "")
        env.pop("ZEST_LOG_PATH", None)
        proc = subprocess.Popen(
            [
                sys.executable,
                "-u",
                "-m",
                "zest.interface.zestd",
                "--host",
                "127.0.0.1",
                "--port",
                "0",
