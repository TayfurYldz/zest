...
class OastCorrelationRepository(Protocol):
    ...
    def list_for_research_run(
        self, research_run_id: str
    ) -> list[OastCorrelationRecord]: ...


class OastCallbackDeliveryRepository(Protocol):
...
class OastAdmissionRepository(Protocol):
...
class PreflightReportRepository(Protocol):
